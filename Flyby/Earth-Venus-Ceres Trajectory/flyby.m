...
% Venus
mu_v = 3.24859e5;               %km^3/s^2   (Gravitational Parameter Venus)
r_v = 6052;                     %km
...
% Grabbing the venus ephemeride file
fid = fopen('venus_flyby_2025_2035_ephemerides.txt', 'r');
if fid == -1 % -1 if file could not be found
    disp('Error in mars file ID');
else 
    temp = textscan(fid, '%f %s %f %f %f %f %f %f', 'Delimiter', ',');
    v_Jt = temp{1};                         % Venus Arrival Times
    v_Cd = temp{2};                         % Venus Arrival Dates (Use for lables on plot)
    v_Pos = [temp{3}, temp{4}, temp{5}];    % Venus Arrival Position
    v_Vel = [temp{6}, temp{7}, temp{8}];    % Venus Arrival Velocity
    clear temp;
    fclose(fid);
end
...
PROCESS FOR CALCULATING C3:
    1. Curve fit C3 for rocket (this case using Delta IV Heavy)
    2. Use lambert problem to calculate the departure velocity at Earth 
    (V_1) and arrival velocity at Venus (V_arr).
    3. Calculate V_inf at Earth
    4. Calculate C3
    5. Check if C3 is <= 80 (this will give us a min delevered mass of
    1710kg)
    6. Use the lambert problem at the arrival date at Venus (from Earths) as
...
% Step 1: C3 for Delta IV Heavy rocket
...
data = [];
tic
% Step 2: Lambert from Earth to Venus
for i = 1:length(e_Jt)
   for j = 1:length(v_Jt)
        if v_Jt(j) > e_Jt(i)
            [V_1, V_2_arrive] = lambert(e_Pos(i,:), v_Pos(j,:), (v_Jt(j)-e_Jt(i))*(24*60*60), "retro", mu_s);
            ...
            if (tempC3 > 0 && tempC3 <= 90)
                
                % Step 6: Lamber from Venus to Ceres
                for k = 1:length(c_Jt)
                    if c_Jt(k) > v_Jt(j)
                        [V_1_depart, V_2] = lambert(v_Pos(j,:), c_Pos(k,:), (c_Jt(k)-v_Jt(j))*(24*60*60), "Pro", mu_s);
                        
                        % Step 7: Calculate V_infinities
                        V_inf_pos = V_1_depart - v_Vel(j, :);
                        V_inf_neg = V_2_arrive - v_Vel(j, :);
                        
                        % Step 8: Make sure V_inf's are close
                        if abs(norm(V_inf_pos)-norm(V_inf_neg)) <= 1 %km/s
                            
                            % Step 9: Calculate del_max
                            del_max = 2*asin(1/(1+((r_v*norm(V_inf_pos)^2)/mu_v)));
                            ...
                            if del < del_max
                                ...
                                % Step 12: Calculate delta_V from Venus
                                dV_assist = 2*norm(V_inf_pos)*sin(del/2);
                                
                                % Step 13: Check if trailing or leading
                                if acos(dot(V_inf_neg, v_Vel(j, :))/(norm(V_inf_neg)*norm(v_Vel(j, :)))) < pi/2
                                    dV_assist = -2*norm(V_inf_pos)*sin(del/2);
                                end
                                ...
                                % data = ['launch date'. 'flyby date', 'arrival date', 'm_delivered', 'deltaV', 'C3', 'dV gained from flyby']
                                data(end+1,1) = e_Jt(i);
                                data(end, 2) = v_Jt(j);
                                data(end, 3) = c_Jt(k);
