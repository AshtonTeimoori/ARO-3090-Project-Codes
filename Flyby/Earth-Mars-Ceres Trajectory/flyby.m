...
data = [];
data_titles = {'iteration', 'launch date', 'flyby date', 'arrival date', 'm_delivered', 'deltaV', 'C3', 'dV gained from flyby'};
data_write = table(data_titles);
writetable(data_write, filename, 'Sheet', 1, 'Range', 'A1');
tic
% Step 2: Lambert from Earth to Mars
for i = 1:length(e_Jt)
   fprintf('Iteration (Outer) %g/%g\n', i, length(e_Jt));
   for j = 1:length(m_Jt)
        if m_Jt(j) > e_Jt(i)
                    ...
                    if c_Jt(k) > m_Jt(j)
                        ...
                        if abs(norm(V_inf_pos)-norm(V_inf_neg)) <= .1 %km/s
                            ...
                            if del < del_max
                                ...
                                data(end, 3) = m_Jt(j);
                                data(end, 4) = c_Jt(k);
                                data(end, 5) = m_delivered(i,k);
                                data(end, 6) = dV_insertion;
                                data(end, 7) = tempC3;
